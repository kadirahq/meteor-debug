...
Store.prototype.trackTime = function(type, timestamp, data) {
  if(!this._clientId) {
    return function() {};
  }
  ...
  var item = [type, timestamp, data];
  currentDataBlock.timechart.push(item);
};
...
Store.prototype._pushToServer = function pushToServer() {
  ...
  _.each(blocks, function(block) {
    ...
    // if only has timechart data,
    // call this method 
    if(!_.isEmpty(block.timechart)) {
      Meteor.call(
        'kadira.debug.updateTimechart', self._browserId,
        self._clientId, block
      );
    }
  });
