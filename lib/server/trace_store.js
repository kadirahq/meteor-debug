...
TraceStore.prototype.registerSession = function(sessionId, browserId, clientId) {
  ...
  // we store registered sessions in an array
...
TraceStore.prototype.unregisterSession = function(browserId, clientId) {
  ...
  // removing from registeredSessions
  var pos = this.registeredSessions.indexOf(sessionId);
  if(pos !== undefined) {
    this.registeredSessions.splice(pos, 1);
  }
};
...
TraceStore.prototype._onMethodTrace = function(trace, session) {
  if(!_.contains(this.registeredSessions, session.id)) {
    // not a valid session,
    // return without build a trace
    return;
  }
