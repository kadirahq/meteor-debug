...
// This keep traces
KdMethodTraces = new Mongo.Collection('kdMethodTraces');
KdSubTraces = new Mongo.Collection('kdSubTraces');
...
TraceStore.prototype.getTrace = function(browserId, clientId, type, id) {
  ...
  if(type === "method") {
    var traceData = KdMethodTraces.findOne({_id: traceKey});
    var trace = JSON.parse(traceData.data);
    return trace;
  } else if(type === "pubsub") {
    var traceData = KdSubTraces.findOne({_id: traceKey});
    var trace = JSON.parse(traceData.data);
    return trace;
  } else {
...
TraceStore.prototype._onMethodTrace = function(trace, session) {
  ...
  // update traceData to DB
  KdMethodTraces.update(
    { _id: key},
