...
TraceStore = function() {
  ...
  this._timeChart = new LRUCache({max: 1000});
};
...
TraceStore.prototype.getTimeChart = function(browserId, clientId, type, id) {
  var key = this._getClientKey(browserId, clientId);
  var sessionId = this._sessionMapper[key];
  if(!sessionId) {
    return;
...
TraceStore.prototype._getTraceKey = function(sessionId, traceId) {
...
TraceStore.prototype._buildTimeChart = function(trace, key) {
  var timeChart = {};
  timeChart.startTime = new Date(trace.at);
  timeChart.metrics = trace.metrics;
  timeChart.type = trace.type;

  this._timeChart.set(key, timeChart);
};