...
Meteor.publish('kadira.debug.client.auth', function(accessToken) {
  ...
  if(AppConfig.env === 'production') {
    ...
    var tokens = (tokensDoc) ? tokensDoc.tokens : null;
    
    if(!tokens || (tokens && tokens.indexOf(accessToken) < 0)) {
      throw new Meteor.Error('401', 'Unauthorized.');
    }
...
KadiraDebug._authorize = function(type, sessionId) {
  ...
  var authSessionsDoc = KdConfig.findOne({_id: docId});
  var authSessions = (authSessionsDoc) ? authSessionsDoc.sessions : null;

  if(authSessions && (authSessions.indexOf(sessionId) > -1)) {
    return true;
  } else {
    throw new Meteor.Error('401', 'Unauthorized.');
  }
