...
AppConfig = {
  ...
  _authorizedSessions: new LRUCache({max: 1000})
};
...
Meteor.publish('kadira.debug.init', function(browserId, clientId, accessToken) {
  ...
  if(kadiraInfo) {
    ...
    if(AppConfig.env === 'production') {
      ...
      AppConfig._authorizedSessions.set(sessionId, true); // ::
    }
...
Meteor.methods({
  'kadira.debug.updateTimeline': function(browserId, clientId, data) {
    ...
    if(AppConfig.env === 'production') {
      ...
      var isValidSession = AppConfig._authorizedSessions.get(sessionId) || false;
      if(!isValidSession) {
        // not a valid session
