...
Meteor.publish('kadira.debug.timeline', function(authKey) {
  // we check autKey only for production apps
  if(AppConfig.env === 'production') {
    if(authKey === AppConfig.authKey) {
...
function getDebugAuthKey() {
  // var defaultAuthKey = generateToken(4);
  var defaultAuthKey = 'AuthKey';
  var authKey = process.env.KADIRA_DEBUG_AUTH_KEY || defaultAuthKey;
  return authKey;
}
