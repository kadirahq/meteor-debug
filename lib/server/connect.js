...
KdData.find({_id: 'serverTimeEvents'}).observeChanges({
  added: function(docId, doc) {
    if(doc[serverId]) {
      var info = JSON.parse(doc[serverId].data);
      var browserId = doc[serverId].browserId;
      var clientId = doc[serverId].clientId;
...
Meteor.methods({
  'kadira.debug.client.updateTimeline': function(browserId, clientId, data) {
    ...
    dataObj[serverId] = {};
    dataObj[serverId].browserId = browserId;
    dataObj[serverId].clientId = clientId;
    dataObj[serverId].data = JSON.stringify(data);
